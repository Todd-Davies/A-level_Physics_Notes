...
\begin{tikzpicture}
	%curve
		\draw (0.5,0) sin (1.5,1) cos (2.5,0) sin (3.5,-1) cos (4.5,0) sin
			(5.5,1) cos (6.5,0) sin (7.5,-1) cos (8.5,0) sin (9.5,1)
	...
	%Amplitude description
		...
		\draw [fill=white,ultra thick,white] (7.6,-0.3) rectangle (9.7,-0.7); 
...
\begin{center}
	...
	\begin{tikzpicture}
		%curve
			\draw (0.55,1) cos (1.5,0) sin (2.5,-1) cos (3.5,0) sin (4.5,1)
			cos (5.5,0) sin (6.5,-1) cos (7.5,0) sin (8.5,1) cos (9.5,0)
...
If you look, you can see that the graph of the velocity is the gradient of the
displacement, and the graph of acceleration is the gradient of the velocity. 

By looking at these graphs, we can also deduce that $a \propto -x$.

\subsection*{Angular frequency in SHM}
The angular frequency of an object undergoing SHM relates to the frequency and
time period via these equations:
\[
	\omega = 2 \pi f
\]
\[
...
\[
	T = \frac{2 \pi}{\omega}
\]
This means you can work out the angular frequency of an object in SHM from a
graph of displacement.

\subsection*{The relationship between acceleration and displacement in SHM}
If we were to draw a graph of acceleration on the y axis and displacement on the
x axis of an object undergoing SHM, it would look like the following:
...
\begin{center}
	\begin{tikzpicture}
		...
		%gradient
			...
			\node at (-0.4,-2.2) {$dx$};
			\draw [-, dashed, thick] (-2,-2) -- (2, -2);
			\node at (2,-2.5) {$\frac{\delta y}{\delta x} 
				= \frac{
					(2 \pi ft)^2 \textrm{\sout{$A \sin(2 \pi ft)$}}
				}{
					\textrm{\sout{$A \sin(2 \pi ft)$}}
				}$};
...
\begin{center}
	\begin{tikzpicture}
		%Graph 1
			%PE
				\draw[dashed] (1.5,2) cos (2,1) sin (2.5,0) cos (3,1)
				sin (3.5,2) cos (4,1) sin (4.5,0) cos (5,1) sin (5.5,2)
				cos (6,1) sin (6.5,0) cos (7,1) sin (7.5,2) cos (8,1)
				sin (8.5,0) cos (9,1) sin (9.5,2) cos (10,1) sin (10.5,0)
				cos (11,1) sin (11.5,2);
			%KE
			\draw (1.5,0) cos (2,1) sin (2.5,2) cos (3,1) sin (3.5,0) cos (4,1)
			sin (4.5,2) cos (5,1) sin (5.5,0) cos (6,1) sin (6.5,2) cos (7,1)
			sin (7.5,0) cos (8,1) sin (8.5,2) cos (9,1) sin (9.5,0) cos (10,1)
			sin (10.5,2) cos (11,1) sin (11.5,0);
			%axis + labels
				\draw [->] (1.5,0) -- (1.5,3);
				\draw [->] (1.5,0) -- (11.7,0);
			...
			%total energy label
				\draw[thick, dotted] (1.5, 2) -- (11.5, 2);
		...
		%Graph 2
			...
			%amplitude
				\draw[dotted, thick] (3.5,-5) -- (3.5,-4.6);
				\node at (3.5,-5.3) {$-A$};
				\draw[dotted, thick] (9.5,-5) -- (9.5,-4.6);
				\node at (9.5,-5.3) {$+A$};
			%total energy
				\draw[dotted, thick] (3.5,-2) -- (9.5,-2);
				\node at (10.2, -1.8) {Total};
				\node at (10.2, -2.2) {Energy};
		%Labels
			%PE label
				\draw[dashed, thick] (4.5, -7) -- (5.5, -7);
...
\begin{itemize}
	...
	\item Potential energy is maximum when the displacement is equal to the
	amplitude of the motion.
	\item At any point, $KE + PE = $\textit{ total energy}
\end{itemize}

\section*{Damped oscillations}
The amplitude of a damped oscillation decreases exponentially with time. This
means that is loses lots of energy at first, but takes a long time to stop
completely.
